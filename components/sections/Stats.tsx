    ...
    <section className="section-pad">
      <div className="mx-auto max-w-[1400px] px-6 md:px-10">
        ...
        <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          ...
          className="font-display italic text-white mb-14" style={{ fontSize: "clamp(28px,4vw,52px)" }}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {stats.map((s, i) => (
            <motion.div key={s.label}
              ...
              <p className="font-display font-bold text-white leading-none" style={{ fontSize: "clamp(34px,4vw,60px)" }}>
                <CounterNumber target={s.value} />
              </p>
              <p className="font-mono text-[11px] uppercase tracking-eyebrow text-white/50 mt-2">{s.label}</p>
